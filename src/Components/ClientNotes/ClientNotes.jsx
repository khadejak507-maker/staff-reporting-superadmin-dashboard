...
import { useCreateNoteByCLientMutation, useDeleteNoteByClientMutation, useGetAllNotesByclientIdQuery, useGetSingleNotesByClientQuery } from "../../redux/Feature/notesApi/clientNoteApi";

const ClientNotes = () => {
  const location = useLocation();
  const selectedUserClient = location.state.selectedUserClient;
  console.log(selectedUserClient)

  const [noteId, setNoteId] = useState();
  const [selectedNote, setSelectedNote] = useState();

  const { data: notesData, refetch } = useGetAllNotesByclientIdQuery({
    clientId: selectedUserClient,
  });

  const { data: singleNote } = useGetSingleNotesByClientQuery(
    { clientId: selectedUserClient, noteId },
    { skip: !noteId }
  );

  const [createNoteByCLient, { isLoading: creating }] = useCreateNoteByCLientMutation();
  const [deleteNoteByClient, { isLoading: deleting }] = useDeleteNoteByClientMutation();
  ...
  const handleAddNote = async () => {
    ...
    try {
      ...
      const res = await createNoteByCLient({ data, clientId: selectedUserClient }).unwrap();
  ...
  const handleDelete = async (id) => {
    try {
      await deleteNoteByClient({ noteId: id }).unwrap();
